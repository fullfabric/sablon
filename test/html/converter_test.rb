...
class HTMLConverterASTTest < Sablon::TestCase
  ...
  def test_i
    ...
    ast = @converter.processed_ast(input)
    assert_equal '<Root: [<Paragraph{Paragraph}: [<Text{}: Lorem >, <Text{italic}: ipsum dolor sit amet>]>]>',
                 ast.inspect
  end
  ...
  def test_br_in_strong
    ...
    assert_equal '[<Text{bold}: Lorem>, <Newline>, <Text{bold}: ipsum>, <Newline>, <Text{bold}: dolor>]',
                 par.runs.inspect
  end
  ...
  def test_br_in_em
    ...
    par = @converter.processed_ast(input).grep(Sablon::HTMLConverter::Paragraph).first
    assert_equal '[<Text{italic}: Lorem>, <Newline>, <Text{italic}: ipsum>, <Newline>, <Text{italic}: dolor>]',
                 par.runs.inspect
  end
  ...
  def test_headings
    ...
    ast = @converter.processed_ast(input)
    assert_equal '<Root: [<Paragraph{Heading1}: [<Text{}: First>]>, <Paragraph{Heading2}: [<Text{}: Second>]>, <Paragraph{Heading3}: [<Text{}: Third>]>]>',
                 ast.inspect
  end
  ...
  def test_h_with_formatting
    ...
    ast = @converter.processed_ast(input)
    assert_equal '<Root: [<Paragraph{Heading1}: [<Text{bold}: Lorem>, <Text{}:  ipsum dolor >, <Text{italic}: sit >, <Text{italic|underline}: amet>]>]>',
                 ast.inspect
  end
  ...
  def test_ul
    ...
    ast = @converter.processed_ast(input)
    assert_equal '<Root: [<Paragraph{ListBullet}: [<Text{}: Lorem>]>, <Paragraph{ListBullet}: [<Text{}: ipsum>]>]>',
                 ast.inspect
  end
  ...
  def test_ol
    ...
    ast = @converter.processed_ast(input)
    assert_equal '<Root: [<Paragraph{ListNumber}: [<Text{}: Lorem>]>, <Paragraph{ListNumber}: [<Text{}: ipsum>]>]>',
                 ast.inspect
  end
