...
class ProcessorDocumentTest < Sablon::TestCase
  ...
  def test_paragraph_block_replacement
    result = process(snippet('paragraph_loop'), { 'technologies' => %w[Ruby Rails] })
  ...
  def test_paragraph_block_within_table_cell
    result = process(snippet('paragraph_loop_within_table_cell'), { 'technologies' => %w[Puppet Chef] })
  ...
  def test_single_row_table_loop
    ...
    result = process(snippet('table_row_loop'),
                     { 'items' => [item.new('1.', 'Milk', '***'), item.new('2.', 'Sugar', '**')] })
  ...
  def test_loop_with_missing_end_raises_error
    ...
    assert_equal 'Could not find end field for «technologies:each(technology)». Was looking for «technologies:endEach»',
                 e.message
  end
  ...
  def test_multi_row_table_loop
    ...
    assert_equal '1. Milk Milk is a white liquid. 2. Sugar Sugar is the generalized name for carbohydrates.',
                 text(result)
  end

  def test_conditional_with_expression
    result = process(snippet('conditional_with_expression'),
                     { 'first_name' => 'Anthony', 'middle_name' => 'Michael', 'last_name' => 'Hall' })
  ...
  def test_conditional_with_expression_and_array_input
  ...
  def test_nested_conditional_expression
    ...
    result = process(snippet('nested_conditional_with_expression'), context.merge({ 'middle_name' => 'Michael' }))

    assert_equal 'Anthony Hall', text(result)

    result = process(snippet('nested_conditional_with_expression'), context.merge({ 'age' => 50 }))

    assert_equal 'Anthony Hall', text(result)

    result = process(snippet('nested_conditional_with_expression'),
                     context.merge({ 'age' => 20, 'middle_name' => 'Michael' }))
  ...
  def test_loop_with_conditional_predicate
    result = process(snippet('paragraph_loop_with_conditional_predicate'), { 'technologies' => %w[Ruby Rails] })
    assert_equal 'Ruby Rails', text(result)
  end
